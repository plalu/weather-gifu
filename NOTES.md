...
>
> **2026-05-16 セッション要約**：クラウド版を GitHub Models で復活 → ローカル版を停止 →
> ずんだもんへ変更 → コードレビューで発見した 5 バグを修正、まで一気に完了。
> コミット 4 本（`bfd9f41`, `6d155b9`, `8a4b466`, `97ee0a1`）+ workflow run 4 回で動作確認済。
...
3. **早朝実行時、`forecast[0]` から今日の最低気温が消える**ことがある。
   `forecast[1]` 週間予報の `tempsMin`/`tempsMax` から **`timeDefines` を今日の日付で照合**して取得する（2026-05-16 修正）。
   旧実装は「先頭エントリ取得」だったため、明日の気温を拾ってしまうリスクがあった。
...
### Wind correction の substring バグ（2026-05-16 修正済）

13. `"強く" in wind_text` は `"やや強く"` にも誤マッチする。
    `wind_text.replace("やや強","")` で除外してから「強く」判定することで解決。
    ユニットテストで `やや強→-7`、`強く→-10`、`非常に強く→-13` を確認済み。
...
- [x] **VOICEVOX 話者を春日部つむぎ → ずんだもんに変更**（2026-05-16、ID 8→3）
- [x] **wind_correction substring バグ修正**（2026-05-16）
- [x] **天気・風テキストの全角スペースを読点に置換**（TTS品質向上、2026-05-16）
